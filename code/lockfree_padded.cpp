    ...
    while(true){
        ...
        if (batch_type == UPDATE){
            for (int op_i = batch_start + tid; op_i < batch_end; op_i += num_threads) {
                ...
                do {
                    expected = ST[u].value.load(std::memory_order_relaxed);
                    desired = combine_fn(expected,x);
                } while (!ST[u].value.compare_exchange_weak(expected, desired, std::memory_order_relaxed));
                ...
                while (u >= u_levels_saved){
                    ...
                    do {
                        expected = ST[u].value.load(std::memory_order_relaxed);
                        left_val = ST[left_child_u].value.load(std::memory_order_relaxed);
                        right_val = ST[right_child_u].value.load(std::memory_order_relaxed);
                        desired = combine_fn(left_val,right_val);
                        if (ST[u].value.compare_exchange_weak(expected, desired, std::memory_order_relaxed)) {
                            ...
                            int left_val_after = ST[left_child_u].value.load(std::memory_order_relaxed);
                            int right_val_after = ST[right_child_u].value.load(std::memory_order_relaxed);
            ...
            for (int level = levels_saved-1; level >= 0; level--){
                ...
                for (int node = tid; node < num_nodes; node+=num_threads){
                    ...
                    ST[u].value.store(
                        combine_fn(ST[leftChild(u)].value.load(std::memory_order_relaxed),ST[rightChild(u)].value.load(std::memory_order_relaxed)),
            ...
            for (int op_i = batch_start + tid; op_i < batch_end; op_i += num_threads) {
                ...
                int query_answer = lockFreePaddedComputeSumCombine(0,i,j,0,array_size,ST,combine_fn, combine_type);
...
void runLockFreePaddedImplementation(const std::vector<int>& batch_starts, const int num_ops, const int num_query, const int num_update, const int levels_saved, const std::vector<std::array<int, 3>>& ops, const int ST_size,
    ...
    for (int tid = 0; tid < num_threads; tid++) {
        ...
        threads.emplace_back(lockFreePaddedWorker, num_threads, tid, array_size, levels_saved, std::ref(ops), std::ref(ST), std::ref(query_results),
                            std::ref(batch_barrier),std::ref(batch_starts),combine_fn, combine_type);
    }
