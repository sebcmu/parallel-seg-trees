...
void coarseWorker(const int num_ops, const int array_size, const std::vector<std::array<int, 3>>& ops, std::vector<int>& ST, std::vector<std::array<int,2>>& query_results, IntCombine combine_fn){
    int query_answer;
    ...
    while(true){
        ...
        if (op[0] == UPDATE){
            ...
            int i = op[1];
            int j = op[2];
            query_answer = computeSumCombine(0,i,j,0,array_size,ST,combine_fn);
...
void runCoarseImplementation(const int num_ops, const int num_query, const int num_update, const std::vector<std::array<int, 3>>& ops, const int ST_size,
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        threads.emplace_back(coarseWorker, num_ops, array_size, std::ref(ops), std::ref(ST), std::ref(query_results), combine_fn);
    }
